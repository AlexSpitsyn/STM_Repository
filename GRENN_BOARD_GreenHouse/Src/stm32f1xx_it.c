...
void EXTI4_IRQHandler(void)
{
  /* USER CODE BEGIN EXTI4_IRQn 0 */
	//EXTI4 - PB4 - TRG_OUT5 - ENC_B - FPCpin9
	if(drv_m1.direction){
		drv_m1.pos++;
		SysCnt.drv_move=0;
		if(IRQ_PRINT_F){
	...
	if(drv_m1.pos==drv_m1.dest_pos){
		...
		//SysVarRW(WR,&SV[vn_DRIVE_POS]); not work in IRQ? too long
		SysState.EEPROM_write_pos_f=1;
