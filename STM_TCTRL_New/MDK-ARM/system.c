...
void SysInit(void) {
  ...
  SysState.error_f = 0;
  //SysState.set_temp = 0; //EEPROM
  SysState.ss_blink_f = 0;
  SysState.ss_update_f = 0;
  //SysState.temp_ctrl_f = 0; //EEPROM
...
void SystemTask(void) {
    ...
    if (!SysState.error_f) {
      ...
      if (drv_m1.run_f) {
        SysCnt.view_mode = 0;
        SysState.view_mode_f = POSITION_VIEW;

        if (Drive_CheckRotation()) {
          DRV_STOP();
          SysState.error_f = 1;
          SysState.error_code |= TIM3 -> CR1;
          drv_m1.dest_pos = drv_m1.position;
        }
      ...
      if (SysCnt.error_check > ERROR_CHECK_TIME) {
        ...
        //Check eeprom
        if (HAL_I2C_IsDeviceReady( & hi2c2, EEPROM_ADDR, 3, HAL_MAX_DELAY)) {
