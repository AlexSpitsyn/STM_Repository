...
uint8_t  EEPROM_Write(uint16_t MEM_ADDR, uint16_t Size, uint8_t * pData)
{
	...
	res = HAL_I2C_Mem_Write (EEPROM_I2C_PORT, (uint16_t) EEPROM_ADDR, MEM_ADDR, I2C_MEMADD_SIZE_8BIT, pData, Size, HAL_MAX_DELAY);
	if (res != HAL_OK) {			
...
uint8_t  EEPROM_Read(uint16_t MEM_ADDR, uint16_t Size, uint8_t * pData)
{
	...
	res = HAL_I2C_Mem_Read (EEPROM_I2C_PORT, (uint16_t) EEPROM_ADDR, MEM_ADDR, I2C_MEMADD_SIZE_8BIT, pData, Size, HAL_MAX_DELAY);	 
	if (res != HAL_OK) {			
		 if(EEPROM_DBG_PRINT_F){
				dbg_print("EEPROM: READ ERROR!\r\n");
				sprintf(dbg_str, "I2C STATUS CODE: %d\r\n", res);
